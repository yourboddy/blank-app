...
def feedback(split, exercise):
    ...
    for idx, achieved in enumerate(latest['Range raggiunto per serie']):
        fb += f"Serie {idx+1}: {'✅ Rientra nel range' if achieved else '❌ Fuori dal range'} (Peso: {latest['Peso per serie (kg)'].strip('[]').split(',')[idx]} kg)\n"
...
with st.form("workout_form"):
    ...
    for i in range(5):
        ...
        with col2:
            reps = st.number_input(f"Serie {i+1} Ripetizioni", min_value=0, value=0, step=1, key=f"rep_{i}")
        with col3:
            min_rep = st.number_input(f"Serie {i+1} min", min_value=0, value=8, step=1, key=f"min_{i}")
        with col4:
            max_rep = st.number_input(f"Serie {i+1} max", min_value=min_rep, value=12, step=1, key=f"max_{i}")
    ...
    rest_time = st.number_input("Tempo medio di recupero tra le serie (s)", min_value=0, value=90, step=1)
